...
window.makeHead = ( data, logo = true ) => {
  ...
  let rowDOM = jQuery('<tr/>', {
    "class": 'some-class',
  })

  logo && rowDOM.append(`<th class="text-center">${"Logo"}</th>`)

  for( let i = 0; i < data.length; i ++ ) {
    rowDOM.append(`<th class="text-center">${data[i]}</th>`)
  }

  return tHead.append(rowDOM)
}
window.makeBody = ( data, logo = true ) => {
  ...
  for( let i = 0; i < data.length; i ++ ) {
    ...
    if (logo) {
      let poolName = row[0].trim()
      let poolSrc = Object(logos.find((item) => poolName === item.name)).src
      rowDOM.append(`<td><img class="pool-logo" src="${poolSrc}" alt="pool-logo" /></td>`)
    }
    
    for( let j = 0; j < row.length; j ++ ) {
      rowDOM.append(`<td>${row[j]}</td>`)
    }
...
window.styled_connect_wallet = (text = "") => {
  return jQuery('<div/>', {
    class: 'd-inline-block left-icon-btn ps-4 pe-2 mr-2 align-items-center',
  })
    .append('<img src="/bsc.svg" alt="bsc" />')
    .append(`<span>${text}</span>`)
}

  
window.styled_clear_browser = jQuery('<div/>', {
  class: 'd-inline-block left-icon-btn ps-4 pe-2 mr-2 align-items-center',
})
  .append('<img src="https://www.freeiconspng.com/uploads/clear-icon-3.png" alt="clear" />')
  .append('<span>Clear Browser Storage</span>')


const buildHead = ( data, logo = true ) => {
  let tHead = jQuery('<thead/>', {
    id: 'some-id',
    "class": 'some-class',
    title: 'now this div has a title!'
...
const buildBody = ( data, logo = true ) => {
  let tBody = jQuery('<tbody/>', {
    id: 'some-id',
    "class": 'some-class',
    title: 'now this div has a title!'
...
window.buildTable = (id, tableData, isLogo = true, className = "") => {
  let tableDOM = jQuery('<table/>', {
    id: id,
    class: className,
  }).append(buildCaption(tableData.title))
    .append(buildHead(tableData.heading, isLogo))
    .append(buildBody(tableData.rows, isLogo))
